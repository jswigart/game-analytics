...
public static class GameAnalyticsUtilities
{
...
public class ProfileBlock : System.IDisposable
{
...
{
    static int GetPowerFromNum(int width)
    {
        int pow = 0;
        while ((1 << pow) < width)
            pow++;
    ...
    public static RenderTexture Reduce(RenderTexture diff, Material mat)
    {
        int reductions = GetPowerFromNum(diff.width);
        var rtList = new List<RenderTexture>();
        int smallRes = diff.width / 2;
        var bigRT = diff;
        for (int i = 0; i < reductions; i++)
        {
            var smallRT = RenderTexture.GetTemporary(smallRes, smallRes, 0, RenderTextureFormat.ARGBFloat);
            rtList.Add(smallRT);
            Graphics.Blit(bigRT, smallRT, mat);
            bigRT = smallRT;
            smallRes >>= 1;
        }
