...
template<typename T>
struct ScopeSetReset
{
	ScopeSetReset( T& valueRef, T setValue )
		: mRef( valueRef )
	{
		mResetValue = mRef;
		mRef = setValue;
...
/*!
Update the property with the value from the object

\sa value()
*/
void QtVariantProperty::UpdatePropertyFromMeta()
{
	QVariant obj = getUserData( "Object" );
	QVariant prop = getUserData( "MetaProperty" );
...
public:
	...
	void slotRangeChanged( QtProperty *property, const QVector3D &min, const QVector3D &max );
...
void QtVariantPropertyManagerPrivate::slotRangeChanged( QtProperty *property, const QVector3D &min, const QVector3D &max )
{
	if ( QtVariantProperty *varProp = m_internalToProperty.value( property, 0 ) )
	{
		emit q_ptr->attributeChanged( varProp, m_minimumAttribute, QVariant( min ) );
	...
	: QtAbstractPropertyManager( parent ), d_ptr( new QtVariantPropertyManagerPrivate )
{
	...
	QtVector3dPropertyManager *vec3PropertyManager = new QtVector3dPropertyManager( this );
	d_ptr->m_typeToPropertyManager[ QMetaType::QVector3D ] = vec3PropertyManager;
	d_ptr->m_typeToValueType[ QMetaType::QVector3D ] = QMetaType::QVector3D;
	d_ptr->m_typeToAttributeToAttributeType[ QMetaType::QVector3D ][ d_ptr->m_minimumAttribute ] = QMetaType::QVector3D;
	d_ptr->m_typeToAttributeToAttributeType[ QMetaType::QVector3D ][ d_ptr->m_maximumAttribute ] = QMetaType::QVector3D;
	d_ptr->m_typeToAttributeToAttributeType[ QMetaType::QVector3D ][ d_ptr->m_decimalsAttribute ] = QVariant::Int;
	connect( vec3PropertyManager, SIGNAL( valueChanged( QtProperty*, QVector3D ) ),
		this, SLOT( slotValueChanged( QtProperty*, QVector3D ) ) );
	connect( vec3PropertyManager, SIGNAL( rangeChanged( QtProperty*, QVector3D, QVector3D ) ),
		this, SLOT( slotRangeChanged( QtProperty*, QVector3D, QVector3D ) ) );
	connect( vec3PropertyManager, SIGNAL( decimalsChanged( QtProperty*, int ) ),
		this, SLOT( slotDecimalsChanged( QtProperty*, int ) ) );
	connect( vec3PropertyManager->subDoublePropertyManager(), SIGNAL( valueChanged( QtProperty*, double ) ),
		this, SLOT( slotValueChanged( QtProperty*, double ) ) );
	connect( vec3PropertyManager->subDoublePropertyManager(), SIGNAL( rangeChanged( QtProperty*, double, double ) ),
		this, SLOT( slotRangeChanged( QtProperty*, double, double ) ) );
	connect( vec3PropertyManager, SIGNAL( propertyInserted( QtProperty*, QtProperty*, QtProperty* ) ),
		this, SLOT( slotPropertyInserted( QtProperty*, QtProperty*, QtProperty* ) ) );
	connect( vec3PropertyManager, SIGNAL( propertyRemoved( QtProperty*, QtProperty* ) ),
		this, SLOT( slotPropertyRemoved( QtProperty*, QtProperty* ) ) );
}
...
QtVariantProperty *QtVariantPropertyManager::addMetaProperty( int propertyType, QObject* obj, const QMetaProperty& metaProperty )
{
	QtVariantProperty * prop = addProperty( propertyType, metaProperty.name() );
	if ( prop == NULL )
		return NULL;
	...
	QVariant value = obj->property( metaProperty.name() );

	bool writable = metaProperty.isWritable();
	if ( writable )
	{
		const QVariant uneditableProperties = obj->property( "uneditableProperties" );
		QVariantList propList = uneditableProperties.toList();
		foreach( const QVariant& item, propList )
		{
			if ( metaPropName == item.toString() )
			{
				writable = false;
				break;
			}
	...
	prop->setUserData( "Object", QVariant::fromValue( obj ) );
	prop->setUserData( "MetaProperty", QVariant( metaProperty.propertyIndex() ) );
	prop->setEnabled( writable );
	prop->setValue( value );
	prop->setToolTip( value.typeName() );

	// register notifications if the value changes if it is has a notifications
	if ( metaProperty.hasNotifySignal() )
	{
		const int slotIndex = prop->metaObject()->indexOfMethod( "UpdatePropertyFromMeta()" );
		if ( slotIndex >= 0 )
		{
			// the notify should be the type of the 
			connect( obj, metaProperty.notifySignal(), prop, prop->metaObject()->method( slotIndex ), Qt::DirectConnection );
		}
	}
	return prop;
}
...
void QtVariantPropertyManager::setValue( QtProperty *property, const QVariant &val )
{
	...
	else if ( QtVector3dPropertyManager *vec3Manager = qobject_cast<QtVector3dPropertyManager *>( manager ) )
	{
		vec3Manager->setValue( internProp, qvariant_cast<QVector3D>( val ) );
		return;
	}
}
	...
	const QString &attribute, const QVariant &value )
{
	...
	{
		if ( attribute == d_ptr->m_maximumAttribute )
			vec3Manager->setMaximum( internProp, qvariant_cast<QVector3D>( value ) );
		if ( attribute == d_ptr->m_minimumAttribute )
			vec3Manager->setMinimum( internProp, qvariant_cast<QVector3D>( value ) );
		if ( attribute == d_ptr->m_decimalsAttribute )
			vec3Manager->setDecimals( internProp, qvariant_cast<int>( value ) );
		return;
	}	
}
...
void QtVariantEditorFactory::connectPropertyManager( QtVariantPropertyManager *manager )
{
	...
	QList<QtVector3dPropertyManager *> vec3PropertyManagers = manager->findChildren<QtVector3dPropertyManager *>();
	QListIterator<QtVector3dPropertyManager *> itVec3( vec3PropertyManagers );
	while ( itVec3.hasNext() )
		d_ptr->m_doubleSpinBoxFactory->addPropertyManager( itVec3.next()->subDoublePropertyManager() );
...
void QtVariantEditorFactory::disconnectPropertyManager( QtVariantPropertyManager *manager )
{
	...
	QListIterator<QtVector3dPropertyManager *> itVec3( vec3PropertyManagers );
	while ( itVec3.hasNext() )
		d_ptr->m_doubleSpinBoxFactory->removePropertyManager( itVec3.next()->subDoublePropertyManager() );
}
