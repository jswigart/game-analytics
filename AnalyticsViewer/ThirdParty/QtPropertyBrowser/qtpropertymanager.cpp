...
template <class SizeValue>
static SizeValue qBoundVec( const SizeValue &minVal, const SizeValue &val, const SizeValue &maxVal, int dimensions )
{
	SizeValue croppedVal = val;
	...
	{
		if ( minVal[ i ] > val[ i ] )
			croppedVal[ i ] = minVal[ i ];
		else if ( maxVal[ i ] < val[ i ] )
			croppedVal[ i ] = maxVal[ i ];
	}
...
QVector3D qBound( QVector2D minVal, QVector2D val, QVector2D maxVal )
{
	return qBoundVec( minVal, val, maxVal, 2 );
}

QVector3D qBound( QVector3D minVal, QVector3D val, QVector3D maxVal )
{
	return qBoundVec( minVal, val, maxVal, 3 );
}

QVector4D qBound( QVector4D minVal, QVector4D val, QVector4D maxVal )
{
	return qBoundVec( minVal, val, maxVal, 4 );
}
...
namespace
{
	...
	namespace
	{
		...
		template <class Value>
		static void orderVecBorders( Value &minVal, Value &maxVal, int dimensions )
		{
			Value fromSize = minVal;
			Value toSize = maxVal;
		...
		void orderBorders( QVector3D &minVal, QVector3D &maxVal )
		{
			orderVecBorders( minVal, maxVal, 3 );
		}
...
template <class PrivateData, class Value>
static void setVectorMinimumData( PrivateData *data, const Value &newMinVal, int dimensions )
{
	data->minVal = newMinVal;

	for ( int i = 0; i < dimensions; ++i )
	{
		if ( data->maxVal[ i ] < data->minVal[ i ] )
			data->maxVal[ i ] = data->minVal[ i ];

		if ( data->val[ i ] < data->minVal[ i ] )
			data->val[ i ] = data->minVal[ i ];
	}
}

template <class PrivateData, class Value>
static void setVectorMaximumData( PrivateData *data, const Value &newMaxVal, int dimensions )
{
	data->maxVal = newMaxVal;

	for ( int i = 0; i < dimensions; ++i )
	{
		if ( data->minVal[ i ] > data->maxVal[ i ] )
			data->minVal[ i ] = data->maxVal[ i ];

		if ( data->val[ i ] > data->maxVal[ i ] )
			data->val[ i ] = data->maxVal[ i ];
	}
}
...
public:
	...
	struct Data
	{
		Data()
			: val( QVector3D( 0, 0, 0 ) )
			, minVal( QVector3D( INT_MIN, INT_MIN, INT_MIN ) )
			, maxVal( QVector3D( INT_MAX, INT_MAX, INT_MAX ) )
			, decimals( 2 )
		{
		}
		QVector3D val;
		...
		QVector3D maxVal;
		int decimals;
		QVector3D minimumValue() const
		{
			return minVal;
		}
		QVector3D maximumValue() const
		{
			return maxVal;
		}
		void setMinimumValue( const QVector3D &newMinVal )
		{
			setVectorMinimumData( this, newMinVal, 3 );
		}
		void setMaximumValue( const QVector3D &newMaxVal )
		{
			setVectorMaximumData( this, newMaxVal, 3 );
		}
	};
	...
	QMap<const QtProperty *, QtProperty *> m_propertyToField[ NUM_PROPS ];
	QMap<const QtProperty *, QtProperty *> m_fieldToProperty[ NUM_PROPS ];
};

const char* QtVector3dPropertyManagerPrivate::sPropNames [] =
{
	"x",
	"y",
	"z",
...
void QtVector3dPropertyManagerPrivate::setValue( QtProperty *property, const QVector3D &val )
{
	m_doublePropertyManager->setValue( m_propertyToField[ X ].value( property ), val.x() );
	m_doublePropertyManager->setValue( m_propertyToField[ Y ].value( property ), val.y() );
	m_doublePropertyManager->setValue( m_propertyToField[ Z ].value( property ), val.z() );
}

void QtVector3dPropertyManagerPrivate::setRange( QtProperty *property, const QVector3D &minVal, const QVector3D &maxVal, const QVector3D &val )
{
	m_doublePropertyManager->setRange( m_propertyToField[ X ][ property ], minVal.x(), maxVal.x() );
	m_doublePropertyManager->setValue( m_propertyToField[ X ][ property ], val.x() );

	m_doublePropertyManager->setRange( m_propertyToField[ Y ][ property ], minVal.y(), maxVal.y() );
	m_doublePropertyManager->setValue( m_propertyToField[ Y ][ property ], val.y() );

	m_doublePropertyManager->setRange( m_propertyToField[ Z ][ property ], minVal.z(), maxVal.z() );
	m_doublePropertyManager->setValue( m_propertyToField[ Z ][ property ], val.z() );
}
...
A size property has nested \e x and \e y
subproperties. The top-level property's value can be retrieved
using the value() function, and set using the setValue() slot.
...
/*!
\fn void QtVector3dPropertyManager::rangeChanged(QtProperty *property, const QVector3D &minimum, const QVector3D &maximum)
...
QtVector3dPropertyManager::QtVector3dPropertyManager( QObject *parent )
	: QtAbstractPropertyManager( parent ), d_ptr( new QtVector3dPropertyManagerPrivate )
{
	d_ptr->q_ptr = this;
...
In order to provide editing widgets for the \e x and \e y
properties in a property browser widget, this manager must be
associated with an editor factory.
...
\sa setMinimum(), maximum(), setRange()
*/
QVector3D QtVector3dPropertyManager::minimum( const QtProperty *property ) const
{
...
\sa setMaximum(), minimum(), setRange()
*/
QVector3D QtVector3dPropertyManager::maximum( const QtProperty *property ) const
{
...
{
	setValueInRange<const QVector3D &, QtVector3dPropertyManagerPrivate, QtVector3dPropertyManager, QVector3D>( this, d_ptr.data(),
		&QtVector3dPropertyManager::propertyChanged,
		&QtVector3dPropertyManager::valueChanged,
		property, val, &QtVector3dPropertyManagerPrivate::setValue );
}
...
void QtVector3dPropertyManager::setDecimals( QtProperty *property, int prec )
{
	...
	for ( int i = 0; i < QtVector3dPropertyManagerPrivate::NUM_PROPS; ++i )
		d_ptr->m_doublePropertyManager->setDecimals( d_ptr->m_propertyToField[ i ][ property ], prec );
...
\sa minimum(), setRange(), rangeChanged()
*/
void QtVector3dPropertyManager::setMinimum( QtProperty *property, const QVector3D &minVal )
{
	setBorderValue<const QVector3D &, QtVector3dPropertyManagerPrivate, QtVector3dPropertyManager, QVector3D, QtVector3dPropertyManagerPrivate::Data>( this, d_ptr.data(),
		&QtVector3dPropertyManager::propertyChanged,
		&QtVector3dPropertyManager::valueChanged,
		&QtVector3dPropertyManager::rangeChanged,
		property,
		&QtVector3dPropertyManagerPrivate::Data::minimumValue,
		&QtVector3dPropertyManagerPrivate::Data::setMinimumValue,
		minVal, &QtVector3dPropertyManagerPrivate::setRange );
}
...
\sa maximum(), setRange(), rangeChanged()
*/
void QtVector3dPropertyManager::setMaximum( QtProperty *property, const QVector3D &maxVal )
{
	setBorderValue<const QVector3D &, QtVector3dPropertyManagerPrivate, QtVector3dPropertyManager, QVector3D, QtVector3dPropertyManagerPrivate::Data>( this, d_ptr.data(),
		&QtVector3dPropertyManager::propertyChanged,
		&QtVector3dPropertyManager::valueChanged,
		&QtVector3dPropertyManager::rangeChanged,
		property,
		&QtVector3dPropertyManagerPrivate::Data::maximumValue,
		&QtVector3dPropertyManagerPrivate::Data::setMaximumValue,
		maxVal, &QtVector3dPropertyManagerPrivate::setRange );
}

/*!
\fn void QtVector3dPropertyManager::setRange(QtProperty *property, const QVector3D &minimum, const QVector3D &maximum)
...
\sa setMinimum(), setMaximum(), rangeChanged()
*/
void QtVector3dPropertyManager::setRange( QtProperty *property, const QVector3D &minVal, const QVector3D &maxVal )
{
	setBorderValues<const QVector3D &, QtVector3dPropertyManagerPrivate, QtVector3dPropertyManager, QVector3D>( this, d_ptr.data(),
		&QtVector3dPropertyManager::propertyChanged,
		&QtVector3dPropertyManager::valueChanged,
		&QtVector3dPropertyManager::rangeChanged,
		property, minVal, maxVal, &QtVector3dPropertyManagerPrivate::setRange );
}
...
void QtVector3dPropertyManager::initializeProperty( QtProperty *property )
{
	...
	for ( int i = 0; i < QtVector3dPropertyManagerPrivate::NUM_PROPS; ++i )
	{
		...
		prop->setPropertyName( QtVector3dPropertyManagerPrivate::sPropNames[ i ] );
		d_ptr->m_doublePropertyManager->setDecimals( prop, decimals( property ) );
		d_ptr->m_doublePropertyManager->setValue( prop, 0 );
		d_ptr->m_doublePropertyManager->setMinimum( prop, 0 );
		d_ptr->m_propertyToField[ i ][ property ] = prop;
		d_ptr->m_fieldToProperty[ i ][ prop ] = property;
		property->addSubProperty( prop );
	}
}
...
void QtVector3dPropertyManager::uninitializeProperty( QtProperty *property )
{
	for ( int i = 0; i < QtVector3dPropertyManagerPrivate::NUM_PROPS; ++i )
	{
		QtProperty * prop = d_ptr->m_propertyToField[ i ][ property ];
		if ( prop )
		{
			d_ptr->m_fieldToProperty[ i ].remove( prop );
			delete prop;
		}
